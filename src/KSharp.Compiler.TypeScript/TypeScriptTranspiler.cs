...
public class TypeScriptTranspiler
{
    ...
    {
        var statements = block.Statements.Select(TranspileStatement).ToImmutableArray();
    ...
    private TsStatement TranspileStatement(IrStatement statement)
    {
        ...
        if (typeName == "IrBlock")
        {
            ...
            if (statements != null)
            {
                var tsStatements = statements.Select(TranspileStatement).ToImmutableArray();
        ...
        if (statement is IrReturnStatement returnStmt)
            return new TsReturnStatement(
                returnStmt.Expression != null ? TranspileExpression(returnStmt.Expression) : null
            );
    ...
    private TsIfStatement TranspileIfStatement(IrIfStatement ifStmt)
    {
        ...
        if (ifStmt.ElseBlock != null)
        {
            ...
            if (
                ifStmt.ElseBlock.Statements.Count == 1
    ...
    private TsExpression TranspileExpression(IrExpression expression)
    {
        ...
        throw new NotImplementedException(
            $"Expression type {expression.GetType().Name} not supported"
        );
    }
    ...
    private static TsExpression TranspileLiteral(IrLiteralExpression literal)
    {
        ...
        throw new NotImplementedException(
            $"Literal type {literal.Value?.GetType().Name} not supported"
        );
    }
    ...
    private TsBinaryExpression TranspileBinaryExpression(IrBinaryExpression binary)
    {
        ...
        var op = binary.Operator switch
        {
            ...
            _ => throw new NotImplementedException(
                $"Operator type {binary.Operator.GetType().Name} not supported"
            ),
        };
    ...
    {
        if (op == IrIntrinsicOperator.Plus)
            return new TsBinaryOperator("+");
        if (op == IrIntrinsicOperator.Minus)
            return new TsBinaryOperator("-");
        if (op == IrIntrinsicOperator.Multiply)
            return new TsBinaryOperator("*");
        if (op == IrIntrinsicOperator.Divide)
            return new TsBinaryOperator("/");
        if (op == IrIntrinsicOperator.Modulo)
            return new TsBinaryOperator("%");
        if (op == IrIntrinsicOperator.Equal)
            return new TsBinaryOperator("===");
        if (op == IrIntrinsicOperator.NotEqual)
            return new TsBinaryOperator("!==");
        if (op == IrIntrinsicOperator.LessThan)
            return new TsBinaryOperator("<");
        if (op == IrIntrinsicOperator.LessThanOrEqual)
            return new TsBinaryOperator("<=");
        if (op == IrIntrinsicOperator.GreaterThan)
            return new TsBinaryOperator(">");
        if (op == IrIntrinsicOperator.GreaterThanOrEqual)
            return new TsBinaryOperator(">=");
        if (op == IrIntrinsicOperator.And)
            return new TsBinaryOperator("&&");
        if (op == IrIntrinsicOperator.Or)
            return new TsBinaryOperator("||");
        if (op == IrIntrinsicOperator.BitwiseAnd)
            return new TsBinaryOperator("&");
        if (op == IrIntrinsicOperator.BitwiseOr)
            return new TsBinaryOperator("|");
        if (op == IrIntrinsicOperator.BitwiseXor)
            return new TsBinaryOperator("^");
    ...
    {
        var arguments = invocation.Arguments.Select(TranspileExpression).ToImmutableArray();
