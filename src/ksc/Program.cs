...
namespace KSharp.Compiler.CommandLine;

class Program : ConsoleAppBase
{
    [Command("build", "Compiles K# source files to the target language")]
    public async Task Build(
        [Option("i", "Input directory containing K# source files")] string input,
        
        [Option("t", "Target language (TypeScript or CSharp)")] 
        Target target = Target.TypeScript,
        
        [Option("o", "Output directory")] 
        string output = "output",
        
        [Option("p", "Generate package.json for Node.js projects")]
        bool generatePackageJson = false,
        
        [Option("ts", "Generate tsconfig.json for TypeScript projects")]
    ...
    private async Task CompileToTypeScript(
        string inputPath, 
        string outputPath, 
